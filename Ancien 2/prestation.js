...
const animalName = params.get("name") || params.get("key") || "";
const isNew    = params.get("new") === "1";
const editIdx  = params.get("idx") !== null ? parseInt(params.get("idx")) : null;

const profiles = getProfiles();
const animal   = profiles[animalName] || {};
let prests     = [...(animal.prestations || [])]; // compatible owner key

let form = isNew ? {} : (prests[editIdx] || {});

const backUrl = `fiche.html?key=${encodeURIComponent(animalName)}`;
