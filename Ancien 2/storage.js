...
// Retourne les propriétaires triés alphabétiquement
function getSortedOwners() {
  const profiles = getProfiles();
  return Object.keys(profiles).sort((a, b) => a.localeCompare(b, "fr"));
}

// Retourne les animaux d'un propriétaire triés alphabétiquement
...
function getAllPrestations() {
  const profiles = getProfiles();
  const all = [];
  Object.entries(profiles).forEach(([ownerKey, owner]) => {
    // Nouveau format : fiches par propriétaire
    if (owner.animals) {
      (owner.prestations || []).forEach(pr => {
        const animalNames = (owner.animals||[]).map(a=>a.name).join(", ");
        const animalTypes = (owner.animals||[]).flatMap(a=>a.animal_type||[]);
        all.push({ ...pr, ownerKey, ownerName: owner.owner_name||ownerKey, animalName: animalNames, animalTypes });
      });
    } else {
      // Ancien format : fiche par animal (rétrocompatibilité)
      (owner.prestations || []).forEach(pr => {
        all.push({ ...pr, animalName: owner.name||ownerKey, animalTypes: owner.animal_type });
      });
    }
  });
...
function getNextKeyRdvs() {
  ...
  Object.entries(profiles).forEach(([key, p]) => {
    ...
    (p.prestations || []).forEach(pr => {
      // RDV récupération — seulement si la date n'est pas passée
      if (!pr.keys_picked_up && (pr.keys_pickup_rdv || pr.keys_pickup_date)) {
        const d = pr.keys_pickup_date ? new Date(pr.keys_pickup_date) : null;
        if (!d || d >= today)
          rdvs.push({ nom, types, rdv:pr.keys_pickup_rdv, date:pr.keys_pickup_date, time:pr.keys_pickup_time, type:"récupération" });
      }
      // RDV restitution — seulement si la date n'est pas passée
      if (pr.keys_picked_up && !pr.keys_returned && (pr.keys_rdv || pr.keys_rdv_date)) {
        const d = pr.keys_rdv_date ? new Date(pr.keys_rdv_date) : null;
        if (!d || d >= today)
          rdvs.push({ nom, types, rdv:pr.keys_rdv, date:pr.keys_rdv_date, time:pr.keys_rdv_time, type:"restitution" });
      }
    });
  });
  rdvs.sort((a,b) => {
...
function getAnimalsByDate(ds) {
  const profiles = getProfiles();
  const res = [];
  Object.entries(profiles).forEach(([ownerKey, p]) => {
    (p.prestations || []).forEach(pr => {
      if (!pr.date_start || !pr.date_end) return;
      const s = new Date(pr.date_start), e = new Date(pr.date_end), d = new Date(ds);
      if (d >= s && d <= e) {
        if (p.animals) {
          // Nouveau format par propriétaire
          const names = (p.animals||[]).map(a=>a.name).join(", ");
          const types = (p.animals||[]).flatMap(a=>a.animal_type||[]);
          res.push({ animalName: names, animalTypes: types, visitTime: pr.visit_time });
        } else {
          res.push({ animalName: p.name||ownerKey, animalTypes: p.animal_type, visitTime: pr.visit_time });
        }
      }
    });
  });
